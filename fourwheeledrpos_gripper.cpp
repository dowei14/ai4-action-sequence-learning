...
//  dsw
    if(conf.useGripper){
      gripper = new Box(0.1 , 0.1, length+0.7*width);
      gripper->setTexture("Images/tire_full.rgb");
      grippertrans = new Transform(objects[0], gripper,
                                  Matrix::translate(width*0.6-radius + 0.2, - 0.2, 0.5));
      grippertrans->init(odeHandle, 0, osgHandle);
      gripper2 = new Box(0.1 , 0.1, length+0.7*width);
      gripper2->setTexture("Images/tire_full.rgb");
      grippertrans2 = new Transform(objects[0], gripper2,
                                  Matrix::translate(width*0.6-radius + 0.2, 0.2, 0.5));
      grippertrans2->init(odeHandle, 0, osgHandle);


//	GripperConf grippConf = Gripper::getDefaultConf();
//	gripperMat->attach(grippertrans2);
	
//      gripperJoint = new SliderJoint(grippertrans,gripper2,grippertrans->getPosition(), Axis(0,1,0));
//      gripperJoint->init(odeHandle, osgHandle, true, 10, true);
//      joints.push_back(gripperJoint);
